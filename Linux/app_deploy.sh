...
# ============ 初始镜像版本，用于测试是否成功 ============
DOCKER_TEST_IMAGE="mcr.microsoft.com/dotnet/samples:aspnetapp"
...
suggest_next_port() {
  ...
  echo "=> 正在分析 ${SERVER_HOST} 上的 Nginx 配置以推荐端口..." >&2

  # --- 1. 检查该应用是否已有配置文件 ---
  echo "   🔍 检查应用 '${APP_NAME}' 是否已部署..." >&2

  # 查找匹配 APP_NAME 的 conf 文件
  MATCHING_FILES=$(sshpass -p "$SERVER_PASS" ssh \
    -o StrictHostKeyChecking=no \
    "$SERVER" "
      find '${NGINX_DIR}' -type f -name '*_${APP_NAME}.conf' -printf '%f\n' 2>/dev/null
    " | head -n1)

  if [ -n "$MATCHING_FILES" ]; then
    # 从文件名提取端口
    EXISTING_PORT=$(echo "$MATCHING_FILES" | grep -o '^[0-9]\+')

    if [ -n "$EXISTING_PORT" ] && [ "$EXISTING_PORT" -ge 1024 ] && [ "$EXISTING_PORT" -le 65535 ]; then
      echo "   ✅ 发现 '${APP_NAME}' 已部署，配置文件: ${MATCHING_FILES}，使用端口: ${EXISTING_PORT}" >&2
      echo $EXISTING_PORT  # ✅ 只有这一行是 stdout，会被 $(...) 捕获
      return
    fi
  fi
  ...
  echo "=> 推荐端口: ${NEXT_PORT}" >&2
  echo $NEXT_PORT  # ✅ 唯一的 stdout 输出
}
...
while true; do
  ...
  if [[ "$PORT_HOST" =~ ^[0-9]+$ ]] && [ "$PORT_HOST" -ge 1 ] && [ "$PORT_HOST" -le 65535 ]; then
    break
  else
    echo "❌ 请输入有效端口 (1-65535)"
  fi
done
